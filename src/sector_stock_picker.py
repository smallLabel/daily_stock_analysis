...
from src.strategies.strategies import MacdStrategy, RsiMeanReversionStrategy, MaTrendStrategy
...
class SectorStockPicker:
    ...
    def analyze_sectors(self, top_n: int = 10) -> List[SectorInfo]:
        """
        分析所有板块，返回最具潜力的板块
    ...
    def _get_all_sectors(self) -> List[Dict]:
        """
        获取所有板块数据
    ...
    def recommend_stocks_from_sector(self, 
                                     sector_info: SectorInfo, 
                                     top_n: int = 5) -> List[StockRecommendation]:
        """
        ...
        try:
            # 1. 获取板块内的所有个股
            stocks = self._get_sector_stocks(sector_info)
            if not stocks:
    ...
    def run_full_analysis(self, 
                         sector_top_n: int = 10, 
                         stock_top_n: int = 5) -> Dict[str, Any]:
        """
        执行完整的板块选股分析
        """
        logger.info("=" * 60)
        logger.info("开始执行完整的板块选股分析")
        logger.info("=" * 60)
        ...
        for sector in sectors[:5]:
            stocks = self.recommend_stocks_from_sector(sector, top_n=stock_top_n)
            if stocks:
