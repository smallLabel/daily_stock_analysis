...
from ..core.base import BaseFetcher, DataFetchError, RateLimitError, STANDARD_COLUMNS
                    ...
                    top_sectors = [
                        {
                            ...
                            'money_flow': safe_float(row.get('资金流入', 0)) / 1e8,  # 转为亿元
                    ...
                    bottom_sectors = [
                        {
                            ...
                            'code': row.get('板块代码', ''),
                            'change_pct': row[change_col],
                            'money_flow': safe_float(row.get('资金流入', 0)) / 1e8,
                            'stock_count': safe_int(row.get('公司数量', 0)),
                            'up_count': safe_int(row.get('上涨家数', 0)),
                            'down_count': safe_int(row.get('下跌家数', 0))
                        }
    ...
    def get_sector_stocks(self, sector_name: str) -> Optional[List[str]]:
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                logger.info(f"[API返回] 板块 {sector_name} 包含 {len(stock_codes)} 只股票")
                return stock_codes
            
            logger.warning(f"[API返回] 板块 {sector_name} 数据为空")
            return []
